...
applyBranchRule s0 name term truths (path,subtrees) (matched,mU) =
  do { (ansListList0,s1) <- mapThread s0 (stepTree name term truths) subtrees
     ; let blocked [(BlockedP _ _ _,_,_)] = True
           blocked _ = False
           blockedTrees = map snd (filter (blocked . fst) (zip ansListList0 subtrees))
...
matchSubAtPath path sub (Branchx bterm bpath ts) s = do { (ans, s') <- mapThread s (matchSubAtPath path sub) ts
                                                        ; return (and ans, s')}
