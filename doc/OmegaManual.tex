...
special syntax for list-like, natural-number-like, pair-like, and record-like, and unary-increment types.
...
list-like, natural-number-like, record-like, pair-like, and unary-increment types). Each
...
Old-style derivations specify a single tag to be associated with the syntax
extension for a particular dataype. That datatype can use only one type
of syntax extension, and the number of constructor functions of the datatype
...
\subsubsection{Example: Tuples are Lists that Reflect the Type of Their Components}
...
\subsubsection{Example: A $n$-ary Summing Function}
...
\subsubsection{Example: Records and Rows}
...
data Test:: *0 where
  ...
  Next :: Test -> Test
  A:: Test
 deriving syntax(w) List(Nil,Cons) Nat(Zero,Succ) Pair(Pair) Tick(Next) Record(RNil,RCons)
