...
\om\ supports both existential and universal quantified components in {\tt data}
...
Guessing \verb+{x -> 0}+ and \verb+{x -> S z1}+ we get the two
paths:
{ %\small
\begin{verbatim}
   1)   { 3t == 5t }
...
For example conside the proposition {\tt Le} (which is a slight
...
a concrete {\tt d} such that (both) {\tt (Le b d c)} and {\tt (Le d c)}
are satisfied.
...
author of the code know that this clause is unreachable, and to
...
\item As a refinment lemma:  \verb+cond -> Equal f g -> (Equal x t,Equal y s)+ where
{\tt f}, {\tt g}, {\tt s}, and {\tt t} are arbitrary terms, but {\tt x} and {\tt y},
are variables.  The intended semantics of a refinememnt lemma is to add
...
\end{verbatim}
\noindent
The semi colon may only appear before the last element in the square brackets.
...
and {\tt (Prod *0)} are classified by {\tt *1}. For a usefull
...
\section{Quiksort Algorithm in \om}\label{quick}
